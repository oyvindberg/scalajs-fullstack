...
object AkkaHttpServer extends App {
  ...
  val indexRoute: Route =
    get {
      ...
      getFromResourceDirectory("META-INF/resources/webjars/tutorialjvm/0.1.0-SNAPSHOT/") ~
      /* when run from sbt (`tutorialJVM/run`) we find assets through file system */
      getFromDirectory("jvm/target/web/classes/main/META-INF/resources/webjars/tutorialjvm/0.1.0-SNAPSHOT") ~
      AutowireAkkaHttpRoute("api", _.route[Api](apiImpl))

  Http(system)
    .newServerAt("0.0.0.0", 8080)
    .bindFlow(indexRoute)
    .foreach { (sb: ServerBinding) =>
